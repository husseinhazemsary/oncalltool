      ...
      <article class="contact-card">
        ...
        <div class="person">
          ...
          <div class="person-meta">
            ...
            <small>${esc(x.phone)} · ${esc(x.email)}</small>
          </div>
        </div>

        <div class="contact-actions">
          <a class="call-btn" href="tel:${x.phone.replace(/ /g,'')}">☎ Call</a>
          <a class="mail-btn" href="mailto:${x.email}">Email</a>
        </div>
      </article>
    `).join('')
...
function renderWeek() {
  const days = ['Wed 10','Thu 11','Fri 12','Sat 13','Sun 14','Mon 15','Tue 16'];
      ...
      <tbody>
        ${preferred.map((team, teamIndex) => `
          <tr>
            ...
            ${days.map((_, dayIndex) => `
              <td>${people[(dayIndex + teamIndex) % people.length]}</td>
            `).join('')}
          </tr>
        `).join('')}
      </tbody>
...
function swapPreview() {
  const [name, date] = document.querySelector('#swapSelect').value.split('|');

  document.querySelector('#swapPreview').innerHTML = `
    <div class="swap-state">
      <b>BEFORE</b>
      <span>You → Wed, 9 Sep</span>
      <span>${name} → ${date.split(', ')[1]}</span>
    </div>

    <div class="swap-state">
      <b>AFTER</b>
      <span>${name} → Wed, 9 Sep</span>
      <span>You → ${date.split(', ')[1]}</span>
    </div>
  `;
}
